...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        foreach ($carts as $key => $cart) {
            ...
            $qtyProduct = $product->stocks->first()->qty;
            if ($cart['quantity'] > $qtyProduct) {
                return back()->with('error', translate('You cannot enter more than ' . $qtyProduct . ' products !'));
            }
        }
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('order_id') != null) {
                if ($request->payment_option == 'paypal') {
                    ...
                    return $paytm->index();
                } elseif ($request->payment_option == 'cash_on_delivery' || $request->payment_option == 'internet_banking') {
                    ...
                    $user = Auth::user();
                    $order = Order::findOrFail($request->session()->get('order_id'));
                    $response = affiliate_wallet_paid_request($user->id, $order->id);
                    ...
                    if ($response->isValid) {
                        ...
                        flash($response->errors[0]->errorMessage)->error();
                    }
                } else {
                    $order = Order::findOrFail($request->session()->get('order_id'));
    ...
    public function store_shipping_info(Request $request)
    {
        if (Auth::check()) {
            ...
            $data['district_name'] = $address->district_name;
            $data['ward_name'] = $address->ward_name;
    ...
    {   
        ...
        if (Session::has('cart') && count(Session::get('cart')) > 0) {
            ...
            $product_cart = Product::find($object['id']);
                if ($product_cart->user_id == $request->owner_id) {
            ...
            if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
                if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                    ...
                    if ($coupon->type == 'cart_base') {
                        ...
                        $request->session()->put('coupon_discount', $coupon_discount);
                        flash(translate('Coupon has been applied'))->success();
                        // return back()->with('error', translate('Coupon has been applied'));
                    }
                } else {
                    flash(translate('You already used this coupon!'))->warning();
                    // return back()->with('error', ranslate('You already used this coupon!'));
                }
            } else {
                flash(translate('Coupon expired!'))->warning();
                // return back()->with('error', translate('Coupon expired!'));
            }
        } else {
            flash(translate('Invalid coupon!'))->warning();
            // return back()->with('error', translate('Invalid coupon!'));
        }
        return back();
    }
