...
class WalletController extends Controller
{
    public function index()
    {
        ...
        $affiliate_wallet = AffiliateWallet::where('user_id', Auth::user()->id)->first();
        // return view('frontend.user.wallet.index', compact('wallets'));
        return view('frontend-v2.user.wallet.index', compact('wallets', 'affiliate_wallet'));
    }
    ...
    public function affiliate_recharge(Request $request)
    {
        $data['userId'] = Auth::user()->id;
        $data['amount'] = $request->amount;
        $data['phone'] = $request->phone;
        // $data['formFile'] = $request->file('billImage');
        $raw_name =  $request->file('billImage')->getClientOriginalName();
        $file_name = $request->file('billImage')->store('uploads/all');
        $file_path = base_path() . '\\public\\' . $file_name;

        $separator = DIRECTORY_SEPARATOR;

        $file_path = str_replace("/", $separator, $file_path);
        $file_path = str_replace('\\', $separator, $file_path);

        // flash($file_path)->success();
        $file = new \CURLFile($file_path,'image/png',$raw_name);
        $data['formFile'] = $file;
        $response = affiliate_wallet_recharge_request($data);

        if ($response->isValid) {
            flash(translate('Recharge Success'))->success();
        }else{
