...
class HomeController extends Controller
{
    public function login()
    {
        ...
        return view('frontend-v2.auth.user_login');
    }
    ...
    public function registration(Request $request)
    {
        ...
        if($request->has('referral_code')){
            ...
            Cookie::queue('referral_code', 000, 43200);
        }
        return view('frontend-v2.auth.user_registration', compact('referral'));
    }
    ...
    public function dashboard()
    {
        if(Auth::user()->user_type == 'seller'){
            ...
            return view('frontend-v2.user.seller.dashboard');
        }
        elseif(Auth::user()->user_type == 'customer'){
            // return view('frontend.user.customer.dashboard');
            return view('frontend-v2.user.customer.dashboard');
        }
        else {
            abort(404);
        }
    ...
    public function profile(Request $request)
    {
        if(Auth::user()->user_type == 'customer'){
            return view('frontend-v2.user.customer.profile');
        }
        ...
        if (Auth::attempt($credentials)) {
            if (Auth::check()) {
                if (Auth::user()->banned == 1) {
                    ...
                    return redirect()->back()->with('error', translate('Your account has been locked !'));
                } else {
                    return redirect()->route('dashboard');
                }
            }
        } else {
            return redirect()->back()->with('error', translate('Invalid email or password'));
        }
    }
    ...
    public function customer_update_profile(Request $request)
    {
        ...
        if($request->hasFile('avatar')) {
            ...
            $upload->extension = strtolower($request->file('avatar')->getClientOriginalExtension());
    ...
    public function index()
    {
        ...
        return view('frontend-v2.home.index', compact('featuredCategories'));
    }

    public function faq()
    {

        return view('frontend-v2.home.faq');
    }

    public function contactus()
    {

        return view('frontend-v2.home.contact-us');
    }

    public function minigame()
    {
        return view('frontend-v2.home.game');
    }
    public function game()
    {
        return view('frontend-v2.home.minigame-example');
    }
    public function game2()
    {
        $event = "1";
        return view('frontend-v2.home.minigame-example',compact('event'));
    }
    public function game3(Request $id)
    {
        $event = $id;
        return view('frontend-v2.home.minigame',compact('event'));
    }
    public function listingByEvents(Request $request, $id)
    {
        $event = $id;
        return view('frontend-v2.home.minigame-example',compact('event'));
    ...
    {
        $bigCategory = Category::where('id', 5)->first();



        return view('frontend.index');
        // return view('frontend-v2.home.index', compact('bigCategory'));
    }
    ...
    public function trackOrder(Request $request)
    {
        if($request->has('order_code')){
            ...
            if($order != null){
                return view('frontend-v2.order.track_order', compact('order'));
            }
        }
        return view('frontend-v2.order.track_order');
    }
    ...
    public function product(Request $request, $slug)
    {
        ...
        if($detailedProduct!=null && $detailedProduct->published){
            $ratings = Review::where('product_id', $detailedProduct->id);
            $overallRatings = Review::where('product_id', $detailedProduct->id)->select('rating', DB::raw('count(id) as amount'))
                ->groupBy('rating')
                ->orderBy('rating', 'desc')
                ->get();
            $ratingAvg = number_format((float)$ratings->avg('rating'), 1, ".", "");

            $reviews = Review::where('product_id', $detailedProduct->id)->orderBy('created_at', 'desc')->paginate(10);
            //updateCartSetup();
            ...
            // }
            $featuredCategories = Category::where('featured', 1)->get();
            return view('frontend-v2.product.view_details', compact('featuredCategories', 'detailedProduct', 'overallRatings', 'ratingAvg', 'reviews'));
        }
        abort(404);
    }
    ...
    public function shop($slug)
    {
        ...
        if($shop!=null){
            ...
            if ($seller->verification_status != 0){
                ...
                return view('frontend-v2.seller_shop', compact('shop'));
            }
            else{
                return view('frontend-v2.seller_shop_without_verification', compact('shop', 'seller'));
            }
        }
        abort(404);
    }
    ...
    public function filter_shop($slug, $type)
    {
        ...
        if($shop!=null && $type != null){
            return view('frontend-v2.seller_shop', compact('shop', 'type'));
        }
        abort(404);
    }
    ...
    public function all_categories(Request $request)
    {
        ...
        return view('frontend-v2.category.list_all', compact('categories'));
    }
    public function all_brands(Request $request)
    {
        $categories = Category::all();
        return view('frontend-v2.home.brands', compact('categories'));
    ...
    public function seller_product_list(Request $request)
    {
        ...
        return view('frontend-v2.user.seller.products', compact('products', 'search'));
    ...
    public function search(Request $request, $category_id = null, $brand_id = null)
    {
        ...
        $category = $category_id != null ? Category::findOrFail($category_id) : null;
        ...
        $categoryBrandIds = $products->where('brand_id', '<>', null)->pluck('brand_id')->flatten(1)->unique('brand_id')->toArray();
        $categoryBrands = Brand::whereIn('id', $categoryBrandIds);

        $featuredCategories = Category::where('featured', 1)->get();
        return view('frontend-v2.category.list', compact('featuredCategories', 'categoryBrands', 'products', 'category', 'categories', 'query', 'category_id', 'brand_id', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color'));

        // return view('frontend.product_listing', compact('products', 'categories', 'query', 'category_id', 'brand_id', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color'));
    }
    ...
    public function seller_digital_product_list(Request $request)
    {
        ...
        return view('frontend-v2.user.seller.digitalproducts.products', compact('products'));
    }
