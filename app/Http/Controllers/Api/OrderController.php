    ...
    public function processOrder(Request $request)
    {
        ...
        foreach ($order->orderDetails as $orderDetail) {
            if ($orderDetail->product->user->user_type == 'seller') {
                ...
                $seller->admin_to_pay = ($request->payment_type == 'cash_on_delivery' || $request->payment_type == 'internet_banking') ? $seller->admin_to_pay - ($price * $commission_percentage) / 100 : $seller->admin_to_pay + ($price * (100 - $commission_percentage)) / 100;
                $seller->save();
            }
        }
        // clear user's cart
