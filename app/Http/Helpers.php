...
//if ($price==0) {$return 'liên hệ';} elseif {return format_price(convert_price($price));}
        return format_price(convert_price($price));
    }
}
...
if(! function_exists('renderStarRatingv2')){
    function renderStarRatingv2($rating,$maxRating=5) {
        $fullStar = "<small class='fas fa-star'></small>";
        $halfStar = "<small class = 'far fa-star-half-o'></small>";
        $emptyStar = "<small class = 'far fa-star text-muted'></small>";
        $rating = $rating <= $maxRating?$rating:$maxRating;
...
// For Affiliate Wallet
if (! function_exists('get_wallet_amount')) {
    function get_wallet_amount($user_id) {
        $wallet = App\AffiliateWallet::where('user_id', $user_id)->first();
        if($wallet!=null){
            return $wallet->current_purchase_money;
        }

        return 0;
    }
}
if (! function_exists('affiliate_wallet_paid_request')) {
    function affiliate_wallet_paid_request($user_id, $order_id) {
        $fields = array(
            "userId" => $user_id,
            "orderId" => $order_id
        );

        $auth_key = env('AFFILIATE_WALLET_KEY');

        $curl = curl_init();

        curl_setopt_array($curl, array(
          CURLOPT_URL => env("AFFILIATE_WALLET") . "AffiliateWallet/RequestPayment",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_TIMEOUT => 30,
          CURLOPT_SSL_VERIFYHOST => 0,
          CURLOPT_SSL_VERIFYPEER => 0,
...
if (! function_exists('affiliate_wallet_recharge_request')) {
    function affiliate_wallet_recharge_request($data) {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => env("AFFILIATE_WALLET") . "AffiliateWallet/RequestDepositWeb",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_TIMEOUT => 30,
          CURLOPT_SSL_VERIFYHOST => 0,
          CURLOPT_SSL_VERIFYPEER => 0,
