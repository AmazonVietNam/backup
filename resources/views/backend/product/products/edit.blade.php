...
<div class="col-lg-8 mx-auto">
	<form class="form form-horizontal mar-top" action="{{route('products.update', $product->id)}}" method="POST" enctype="multipart/form-data" id="choice_form">
		...
		<div class="col-md-6">
			<div class="row">
				@if ($product->status == config('common.status.pending') || $product->status == config('common.status.reject'))
					<div class="mb-3">
						<button type="button" data-id="{{ $product->id }}" name="button" class="btn btn-success btn-accecpt">{{ translate('Accept') }}</button>
					</div>
				@endif
				<div class="ml-3 mb-3">
					<button type="button" data-id="{{ $product->id }}" data-toggle="modal" data-target="#exampleModal" name="button" class="btn btn-reject btn-danger">{{ translate('Reject') }}</button>
				</div>
	...
	<div class="modal-dialog" role="document">
	  <div class="modal-content">
		...
		<div class="modal-body">
			<textarea class="aiz-text-editor note-product" name="note">{!! isset($product->note) ? $product->note : '' !!}</textarea>
		</div>
		<div class="modal-footer">
		  <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
		  <button type="button" class="btn save-reject btn-primary">Save changes</button>
		</div>
	  </div>
	</div>
</div>
...
<script type="text/javascript">
	const urlHome = '{{ route("products.all")}}';
	...
	$('.btn-reject').click(function() {
		var id = $(this).data('id');
		$('.save-reject').attr('data-id', id);
	});

	$('.save-reject').on('click', function() {
		var content = $('.note-product').val();
		var type = "reject";
		const url = '{{ route("products.update", $product->id) }}';
		$.ajax({
			type: "post",
