		            ...
		            <tbody>
		                @foreach($brands as $key => $brand)
		                    <tr>
		                        ...
		                        <td>{{ $brand->getTranslation('category') }}</td>
		                        <td>{{ $brand->top }}</td>
		                        <td class="text-right">
			...
			<div class="card-body">
				<form action="{{ route('brands.store') }}" method="POST">
					...
					<div class="form-group mb-3">
						...
						<select class="select2 form-control aiz-selectpicker" name="category" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true" required>
                            <option value="Thương Hiệu">{{ translate('brand') }}</option>
                            <option value="Bộ ban ngành">{{ translate('Ministry') }}</option>
                            <option value="Viện & Trường">{{ translate('Institute') }}</option>
                        </select>

					<!--
						<input type="text" placeholder="{{translate('Category')}}" name="category" class="form-control" required>
					-->
					</div>
					<div class="form-group mb-3">
						<label for="name">{{translate('Link')}}</label>
						<input type="text" placeholder="{{translate('Link')}}" name="link" class="form-control">
					</div>
					<div class="form-group mb-3">
						<label for="name">{{translate('Top')}}</label>
						<input type="text" placeholder="{{translate('Top')}}" name="top" class="form-control">
					</div>
					<div class="form-group mb-3">
						<label for="name">{{translate('Logo')}} <small>({{ translate('200x200') }})</small></label>
