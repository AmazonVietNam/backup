        ...
        <div class="col-md-6 mx-auto mb-3" >
            <div class="bg-grad-1 text-white rounded-lg overflow-hidden">
                ...
                <div class="px-3 pt-3 pb-3">
                    <div class="h4 font-weight-bold text-center">{{ max(0, Auth::user()->seller->remaining_uploads) }}</div>
...
<div class="card">
    <div class="card-header d-flex justify-content-between">
        <div>
            <a href="{{ route('seller.products.upload')}}" class="btn btn-primary">
            ...
            <tbody>
                @foreach ($products as $key => $product)
                    <tr>
                        ...
                        <td>
                            @if ($product->status == config('common.status.accept'))
                        ...
                        <td>
                            @if ($product->status == config('common.status.pending'))
                                {{ translate(config('common.lang.status.pending')) }}
                            @elseif ($product->status == config('common.status.accept'))
                                {{ translate(config('common.lang.status.accept')) }}
                            @else
                                {{ translate(config('common.lang.status.reject')) }}
                            @endif
                        </td>
                        <td class="text-right">
