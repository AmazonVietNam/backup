    ...
    <div class="card-body">
        <form action="{{route('support_ticket.seller_store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <textarea class="form-control mb-3" name="reply" data-buttons='[["font", ["bold", "underline", "italic"]],["para", ["ul", "ol"]],["view", ["undo","redo"]]]' required></textarea>
            </div>
                        ...
                        <div class="media">
                            <a class="media-left" href="#">
                                @if($ticketreply->user->avatar_original != null)
                                    <span class="avatar m-3"><img class="u-sm-avatar rounded-circle" src="{{ uploaded_asset($ticketreply->user->avatar_original) }}"></span>
                                @else
                                    <span class="avatar m-3"><img class="u-sm-avatar rounded-circle" src="{{ static_asset('assets/img/avatar-place.png') }}"></span>
                                @endif
                            </a>
                            <div class="media-body">
                                <div class="comment-header">
                                    <span class="font-weight-semi-bold h6">{{ $ticketreply->user->name }}</span>
                    ...
                    <div class="media">
                        <a class="media-left" href="#">
                            @if($ticket->user->avatar_original != null)
                                <span class="avatar m-3"><img class="u-sm-avatar rounded-circle" src="{{ uploaded_asset($ticket->user->avatar_original) }}"></span>
                            @else
                                <span class="avatar m-3"><img class="u-sm-avatar rounded-circle" src="{{ static_asset('assets/img/avatar-place.png') }}"></span>
                            @endif
                        </a>
                        <div class="media-body">
                            <div class="comment-header">
                                <span class="font-weight-semi-bold h6">{{ $ticket->user->name }}</span>
