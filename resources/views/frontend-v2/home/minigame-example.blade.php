...
<div class="container">
    <div class="row">
      ...
      <section id="luckywheel" class="hc-luckywheel">
        ...
        -->
        <div class="col-sm-8">
            @foreach(\App\Events_games::where('id', $event)->take(1)->get() as $events_games)
            <img src="{{uploaded_asset($events_games->banner)}}" style="width: 100%;" />
            @endforeach
        </div>
    </div>
    ...
    <div class="row" id="bg2">
        @foreach(\App\Reward::where('id_eventsgames', $event)->orderBy('stt', 'asc')->take(3)->get() as $reward)
        <div class="col-sm-4">
            <a href="{{$reward->link}}"><img src="{{uploaded_asset($reward->logo)}}" alt="Image Description"></a>
        </div>
        @endforeach
    </div>
  ...
  <tbody>
        ...
        @foreach(\App\Reward::where('stt', '1')->take(1)->get() as $giai)
        <?php $giai_nhat=$giai->id_user ?>
        @endforeach
         @foreach(\App\Reward::where('stt', '2')->take(1)->get() as $giai)
        <?php $giai_nhi=$giai->id_user ?>
        @endforeach
         @foreach(\App\Reward::where('stt', '3')->take(1)->get() as $giai)
        <?php $giai_ba=$giai->id_user ?>
        @endforeach
         @foreach(\App\Reward::where('stt', '4')->take(1)->get() as $giai)
        <?php $giai_tu=$giai->id_user ?>
        @endforeach

        @foreach(\App\User::where('id', $giai_nhat)->take(1)->get() as $user)
        <?php 
            $cut_email = substr($user->email, 0,6);
            $ma_hoa = str_replace($cut_email, '**********', $user->email);
            $cut_phone1 = substr($user->phone, 0,4);
            $cut_phone2 = substr($user->phone, 4,3);
            $cut_phone3 = substr($user->phone, 7,3);
            $ma = str_replace($cut_phone3,'***',$cut_phone3);
            $noi = $cut_phone1.'.'.$cut_phone2.'.'.$ma;
        ?>
    <tr>
        <td>01</td>
        <td>Giải nhất</td>
        ...
        @endforeach
        @foreach(\App\User::where('id', $giai_nhi)->take(1)->get() as $user)
        <?php 
            $cut_email = substr($user->email, 0,6);
            $ma_hoa = str_replace($cut_email, '**********', $user->email);
            $cut_phone1 = substr($user->phone, 0,4);
    ...
    <tr>
        ...
        @endforeach
        @foreach(\App\User::where('id', $giai_ba)->take(1)->get() as $user)
        <?php 
            $cut_email = substr($user->email, 0,6);
            $ma_hoa = str_replace($cut_email, '**********', $user->email);
            $cut_phone1 = substr($user->phone, 0,4);
    ...
    <tr>
        ...
        @endforeach
        @foreach(\App\User::where('id', $giai_tu)->take(1)->get() as $user)
        <?php 
            $cut_email = substr($user->email, 0,6);
            $ma_hoa = str_replace($cut_email, '**********', $user->email);
            $cut_phone1 = substr($user->phone, 0,4);
