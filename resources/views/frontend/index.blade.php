...
@section('content')
                        ...
                        <div class="col-50_" >
                            <div class="mb-3 mb-lg-0">
                                <a href="{{ json_decode(get_setting('home_banner1_links'), true)[$key] }}" class="d-block text-reset">
                                    <img src="{{ static_asset('assets/img/placeholder-rect.jpg') }}" data-src="{{ uploaded_asset($banner_1_imags[$key]) }}" alt="{{ env('APP_NAME') }} promo" class="img-fit lazyload">
                                </a>
                            </div>
                        </div>
                    @endforeach
                                    ...
                                    <div class="p-md-3 p-2 text-center">
                                        <div class="fs-15">
                                            @if(home_base_price($product->id) != home_discounted_base_price($product->id))
                                                <del class="fw-600 opacity-50 mr-1">{{ home_base_price($product->id) }}</del><br/>
                                            @endif
											@if(home_discounted_base_price($product->id)=='0đ')
												<span class="fw-700 text-primary"> Liên hệ </span><br>
	...
	{{-- @if($num_todays_deal > 0)
		<section class="mb-4">
				<div class="container">
					<div class="px-2 py-4 px-md-4 py-md-3 bg-white shadow-sm rounded">
					   <div class="bg-soft-primary rounded-top d-flex align-items-center justify-content-left">
									<span class="fw-600 fs-16 mr-2">
									<img src="https://89bay.com.vn/doc/images/fl.png" class="lazyload img-fit" height="35" onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';">
									</span>
									<span class=""><img src="https://89bay.com.vn/doc/images/cd.png" class="lazyload img-fit" height="35" onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder-rect.jpg') }}';"></span>
						</div>
						<div class="aiz-carousel gutters-10 half-outside-arrow" data-items="6" data-lg-items="6"  data-md-items="2" data-sm-items="2" data-xs-items="1" data-rows="1">
										...
										<div class="row no-gutters box-3 align-items-center border border-light rounded hov-shadow-md my-2 has-transition">
												...
												<a href="{{ route('product', $product->slug) }}" class="d-block p-2 text-reset bg-white h-100 rounded">
													<div class="row gutters-5 align-items-center">
																...
																<img
																	class="lazyload img-fit h-120px h-md-180px"
																	src="{{ static_asset('assets/img/placeholder.jpg') }}"
																	data-src="{{ uploaded_asset($product->thumbnail_img) }}"
																	alt="{{ $product->getTranslation('name') }}"
																	onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';"
													...
													<div class="col-lg">
															<div class="fs-16 align-items-center">
																<center><span class="d-block text-primary fw-600">{{ home_discounted_base_price($product->id) }}</span>
																@if(home_base_price($product->id) != home_discounted_base_price($product->id))
																	<del class="d-block opacity-70">{{ home_base_price($product->id) }}</del><br/>
																@endif
																</center>
															</div>
													</div>
	...
	{{-- List wise Products --}}
    <div id="section_home_list">
			<section class="mb-4">
				<div class="container">
					<div class="col-lg-2" style="height:100%; float:left;">
						<img src="https://89bay.com.vn/doc/images/slide_3.png" data-src="https://89bay.com.vn/doc/images/slide_3.png" alt="amazongroup.vn promo" class="img-fit lazyloaded"> 
					</div>

					<div class="row gutters-10 col-lg-10" style="float: right;">
								@if (get_setting('top10_categories') != null)
										<div class="d-flex mb-3 align-items-baseline border-bottom" style="display:none;">
											<a href="{{ route('categories.all') }}" class="">Xem chi tiết</a>
										</div>
										<div class="row">
											@php $top10_categories = json_decode(get_setting('top10_categories'));$i=0; @endphp
											@foreach ($top10_categories as $key => $value)
												@php $category = \App\Category::find($value); $i++; if($i==9) break; @endphp
												@if ($category != null)
													<div class="col-sm-3">
                                        ...
                                        <div class="aiz-card-box border border-light rounded hov-shadow-md my-2 has-transition">
                                            ...
                                            <div class="p-md-3 p-2 text-center">
                                                <div class="fs-15 mb-1">
													@if(single_price($customer_product->unit_price)=='0đ')
														<span class="fw-700 text-primary"> Liên hệ </span><br>
														<span class="fw-700 text-primary"> -------- </span>
													@else
                        ...
                        <div class="col-xl col-md-6">
                            <div class="mb-3 mb-lg-0">
                                <a href="{{ json_decode(get_setting('home_banner2_links'), true)[$key] }}" class="d-block text-reset">
                                    <img src="{{ static_asset('assets/img/placeholder-rect.jpg') }}" data-src="{{ uploaded_asset($banner_2_imags[$key]) }}" alt="{{ env('APP_NAME') }} promo" class="img-fit lazyload">
                                </a>
                            </div>
                        </div>
					</div>
