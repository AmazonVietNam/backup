            ...
            <div class="my-md-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-3 flex-nowrap flex-xl-wrap overflow-auto overflow-xl-visble">
                        ...
                        <li class="breadcrumb-item flex-shrink-0 flex-xl-shrink-1 active" aria-current="page">{{translate('Delivery Info')}}</li>
                    </ol>
                </nav>
            </div>
            <!-- End breadcrumb -->
                ...
                <form class="form-default" action="{{ route('checkout.store_delivery_info') }}" role="form" method="POST">
                    ...
                    <div class="card mb-3 shadow-sm border-0 rounded">
                        <div class="card-header p-3">
                            <h5 class="fs-16 font-weight-semi-bold mb-0">{{ translate('Products') }}</h5>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="row border-top pt-3">
                                <div class="col-md-6">
                                    <h6 class="fs-15 font-weight-semi-bold">{{ translate('Choose Delivery Type') }}</h6>
                                </div>
                                <div class="col-md-6">
                                        ...
                                        <div class="col-6">
                                            <label class="border rounded p-2 d-block bg-white mb-0">
                                                ...
                                                <span class="d-flex p-3">
                                                    ...
                                                    <span class="flex-grow-1 pl-3 font-weight-semi-bold">{{  translate('Home Delivery') }}</span>
                                                </span>
                                            </label>
                                        </div>
                                        <div class="col-6">
                                            <label class="border rounded p-2 d-block bg-white mb-0">
                                                ...
                                                <span class="d-flex p-3">
                                                    <span class="rounded flex-shrink-0 mt-1"></span>
                                                    <span class="flex-grow-1 pl-3 font-weight-semi-bold">{{  translate('Local Pickup') }}</span>
                                                </span>
                                            </label>
                                        </div>
                                    </div>
                                    <div style="height:80px" class="mt-4 pickup_point_id_admin d-none">
                                        <select
                                            ...
                                            @foreach (\App\PickupPoint::where('pick_up_status',1)->get() as $key => $pick_up_point)
                                                <option
                                                    ...
                                                    data-content="<span class='d-block'>
                                                                    <span class='d-block fs-16 font-weight-semi-bold mb-2'>{{ $pick_up_point->getTranslation('name') }}</span>
                        ...
                        <div class="card-footer d-flex justify-content-end">
                            <button type="submit" name="owner_id" value="{{ App\User::where('user_type', 'admin')->first()->id }}" class="btn font-weight-600 btn-primary">{{ translate('Continue to Payment')}}</a>
                        </div>
                    </div>
                </form>
                @endif
                <form class="form-default"  action="{{ route('checkout.store_delivery_info') }}" role="form" method="POST">
                    ...
                    @if (!empty($seller_products))
                        @foreach ($seller_products as $key => $seller_product)
                            <div class="card mb-3 shadow-sm border-0 rounded">
                                <div class="card-header p-3">
                                    <h5 class="fs-16 font-weight-semi-bold mb-0">{{ \App\Shop::where('user_id', $key)->first()->name }} {{ translate('Products') }}</h5>
                                </div>
                                <div class="card-body">
                                    <ul class="list-group list-group-flush">
                                                        ...
                                                        >
                                                            ...
                                                            @foreach (json_decode(\App\Shop::where('user_id', $key)->first()->pick_up_point_id) as $pick_up_point)
                                                                ...
                                                                <option
                                                                    ...
                                                                    data-content="<span class='d-block'>
                                                                                    <span class='d-block fs-16 font-weight-semi-bold mb-2'>{{ \App\PickupPoint::find($pick_up_point)->getTranslation('name') }}</span>
                                ...
                                <div class="card-footer d-flex justify-content-end">
                                    <button type="submit" name="owner_id" value="{{ $key }}" class="btn font-weight-semi-bold btn-primary">{{ translate('Continue to Payment')}}</a>
                                </div>
                            </div>
                        @endforeach
                    @endif
