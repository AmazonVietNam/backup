    ...
    <section class="pb-4 pt-5">
        <div class="container">
            <div class="text-center">
                <h1 class="heading-3 font-weight-semi-bold">{{__('Premium Packages for Sellers')}}</h1>
            </div>
                ...
                @foreach ($seller_packages as $key => $seller_package)
                    <div class="col-xl-3 col-lg-4 col-md-6">
                        <div class="dashboard-widget text-center mt-4 c-pointer">
                            <img alt="Package Logo" src="{{ static_asset($seller_package->logo) }}" width="200" class="img-fluid mb-4">
                            ...
                            @if ($seller_package->amount == 0)
                                <button class="btn btn-primary w-100" onclick="get_free_package({{ $seller_package->id}})">{{__('Free Package')}}</button>
                            @else
                                <button class="btn btn-primary w-100" onclick="show_price_modal({{ $seller_package->id}})">{{__('Get Package')}}</button>
                            @endif
                        </div>
                    </div>
                @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-zoom product-modal" id="modal-size" role="document">
        <div class="modal-content position-relative">
            <div class="modal-header">
                <h5 class="modal-title font-weight-semi-bold heading-5">{{__('Purchase Your Package')}}</h5>
            ...
            <form class="" id="package_payment_form" action="{{ route('seller_packages.purchase') }}" method="post">
                ...
                <div class="modal-body gry-bg px-3 pt-3">
                    <div class="row">
                        ...
                        <div class="col-md-10">
                            <div class="mb-3">
                                <select class="form-control js-select selectpicker" data-minimum-results-for-search="Infinity" name="payment_option">
